using Idempo.TestKit;
using StackExchange.Redis;

namespace Idempo.StackExchangeRedis.Tests;
...
/// <c>IDEMPO_TEST_REDIS</c> environment variable); see the repo README for how to start one
/// locally with Docker.
/// </summary>
public sealed class RedisIdempotencyStoreTests : IdempotencyStoreContractTests, IDisposable
{
    private static readonly string RedisConnectionString =
        Environment.GetEnvironmentVariable("IDEMPO_TEST_REDIS") ?? "localhost:16379";
    ...
    private readonly string _keyPrefix = $"idempo-tests:{Guid.NewGuid():N}:";
