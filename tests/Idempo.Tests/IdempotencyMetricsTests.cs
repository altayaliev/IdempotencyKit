...
public class IdempotencyMetricsTests
{
    ...
    public void RecordReservation_emits_a_counter_measurement_tagged_with_the_outcome()
    {
        ...
        {
            if (instrument.Meter.Name == IdempotencyMetrics.MeterName && instrument.Name == "idempo.reservations")
            {
        ...
        {
            var outcome = tags.ToArray().FirstOrDefault(t => t.Key == "idempo.outcome").Value?.ToString();
    ...
    public void RecordPurged_emits_the_count_but_skips_zero()
    {
        ...
        listener.InstrumentPublished = (instrument, l) =>
        {
            if (instrument.Meter.Name == IdempotencyMetrics.MeterName && instrument.Name == "idempo.cleanup.purged")
            {
                l.EnableMeasurementEvents(instrument);
            }
        };
