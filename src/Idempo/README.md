...
Core abstractions for [Idempo](https://github.com/altayaliev/Idempo) — zero-config idempotency
for .NET APIs. This package has **no ASP.NET Core dependency**; most users don't install it
directly, since [`Idempo.AspNetCore`](https://www.nuget.org/packages/Idempo.AspNetCore) pulls it
in automatically.
...
You're implementing a custom `IIdempotencyStore` (e.g. for a backend Idempo doesn't ship a
provider for) without using the ASP.NET Core middleware. For the normal case — an ASP.NET Core
API — install [`Idempo.AspNetCore`](https://www.nuget.org/packages/Idempo.AspNetCore) instead,
which brings this package in as a dependency.

## Full documentation

See the [project README](https://github.com/altayaliev/Idempo#readme) — install/quickstart,
...
[Idempo](https://github.com/altayaliev/Idempo) üçün core abstraction-lar — .NET API-ləri üçün
zero-config idempotency. Bu paketin **ASP.NET Core asılılığı yoxdur**; əksər istifadəçilər onu
birbaşa quraşdırmır, çünki [`Idempo.AspNetCore`](https://www.nuget.org/packages/Idempo.AspNetCore)
...
ASP.NET Core middleware istifadə etmədən fərdi bir `IIdempotencyStore` yazırsınızsa (məs. Idempo-nun
provider təklif etmədiyi bir backend üçün). Normal halda — ASP.NET Core API üçün —
[`Idempo.AspNetCore`](https://www.nuget.org/packages/Idempo.AspNetCore) quraşdırın, o bu paketi
asılılıq kimi özü ilə gətirir.

## Tam sənədləşdirmə

[Layihə README-inə](https://github.com/altayaliev/Idempo#readme) baxın — quraşdırma/sürətli
