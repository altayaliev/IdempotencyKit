...
/// <summary>
/// Idempo's <see cref="System.Diagnostics.Metrics.Meter"/>, named <see cref="MeterName"/>. Any
/// OpenTelemetry (or other <c>System.Diagnostics.Metrics</c> listener) setup can observe it by
/// calling <c>.AddMeter("Idempo")</c> — no OpenTelemetry package dependency is added by Idempo
/// itself. Registered as a singleton by <c>AddIdempo()</c>.
/// </summary>
public sealed class IdempotencyMetrics : IDisposable
{
    public const string MeterName = "Idempo";
    ...
    public void RecordReservation(IdempotencyReservationKind kind) =>
        _reservations.Add(1, new KeyValuePair<string, object?>("idempo.outcome", OutcomeTag(kind)));
