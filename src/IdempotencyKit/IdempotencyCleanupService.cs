    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                if (purged > 0)
                {
                    _logger.LogDebug("IdempotencyKit cleanup purged {Count} expired record(s).", purged);
                }
            }
            catch (OperationCanceledException)
            {
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogWarning(ex, "IdempotencyKit cleanup pass failed; will retry in {Interval}.", interval);
            }
        }
    }
}
